...
BALL_POSITION_ARR = [
    #從最左邊拋小球
    {'start_pos': (        0, height/3), 'throw_dir':  1, 'speedx': (  3 ,  5), 'speedy': ( -10 ,  -5), 'hit_time': 0.9},
    #從最右邊拋小球
    {'start_pos': (    width, height/3), 'throw_dir': -1, 'speedx': ( -5 , -3), 'speedy': ( -10 ,  -5), 'hit_time': 0.9}, 
    #從中間 往右拋平球
    {'start_pos': (  width/2, height/3), 'throw_dir':  1, 'speedx': (  3 ,  5), 'speedy': ( -10 ,  -5), 'hit_time': 0.5},
    #從中間 往左拋平球
    {'start_pos': (  width/2, height/3), 'throw_dir': -1, 'speedx': ( -5 , -3), 'speedy': ( -10 ,  -5), 'hit_time': 0.5},
    #從中間 往右拋高遠球 接球點低於中間
    {'start_pos': (  width/2, height/3), 'throw_dir':  1, 'speedx': (  3 ,  5), 'speedy': ( -20 , -15), 'hit_time': 1.1 },
    #從中間 往左拋高遠球 接球點低於中間
    {'start_pos': (  width/2, height/3), 'throw_dir': -1, 'speedx': ( -5 , -3), 'speedy': ( -20 , -15), 'hit_time': 1.1 },
    #從1/4 往直線(稍微偏左)拋高遠球 接球點低於中間
    {'start_pos': (  width/4, height/3), 'throw_dir': -1, 'speedx': ( -2 , -1), 'speedy': ( -20 , -15), 'hit_time': 1.1 },
    #從3/4 往直線(稍微偏右)拋高遠球 接球點低於中間
    {'start_pos': (width*3/4, height/3), 'throw_dir':  1, 'speedx': (  1 ,  2), 'speedy': ( -20 , -15), 'hit_time': 1.1 },
    #從中間 往右拋高遠球 接球點高於中間
    {'start_pos': (  width/2, height/3), 'throw_dir':  1, 'speedx': (  3 ,  5), 'speedy': ( -20 , -15), 'hit_time': 0.6 },
    #從中間 往左拋高遠球 接球點高於中間
    {'start_pos': (  width/2, height/3), 'throw_dir': -1, 'speedx': ( -5 , -3), 'speedy': ( -20 , -15), 'hit_time': 0.6 },
    #從1/4 往直線(稍微偏左)拋高遠球 接球點高於中間
    {'start_pos': (  width/4, height/3), 'throw_dir': -1, 'speedx': ( -2 , -1), 'speedy': ( -20 , -15), 'hit_time': 0.6 },
    #從3/4 往直線(稍微偏右)拋高遠球 接球點高於中間
    {'start_pos': (width*3/4, height/3), 'throw_dir':  1, 'speedx': (  1 ,  2), 'speedy': ( -20 , -15), 'hit_time': 0.6 },

]
# TODO: 可以改成讀取檔案 / 更好的設定方式
...
def initail_new_shuttlecock(start_pos: Tuple[int, int], speed: Tuple[int, int]):
    ...
    if start_pos is Tuple:
        new_shuttlecock = Shuttlecock(start_pos, special_judge= True)
    elif start_pos is None:
        new_shuttlecock = Shuttlecock(basic_set['start_pos'], special_judge= True)
    if speed is Tuple:
        new_shuttlecock.set_speed( speed )
    elif speed is None:
        new_shuttlecock.set_speed(
            pg.math.Vector2( 
                    random.randint( basic_set.get('speedx')[0], basic_set.get('speedx')[1]), 
                    random.randint( basic_set.get('speedy')[0], basic_set.get('speedy')[1])
                        ) 
            )
    new_shuttlecock.set_can_hit_after_throw(basic_set['hit_time'])

    return new_shuttlecock

def stage3(cap, detection):
    global width, height
    # 初始化pygame
    pg.init()
...
#       如果沒球在畫面上 或是 最後一顆球出現1.5秒以上了 就再產生一顆球
            ...
            if shuttlecock.can_hit and len(filtered_results) > 0:
                for row in collision_boxes:
                    if pg.Rect(row[0], row[1], row[2], row[3]).colliderect(shuttlecock.rect):
                        ...
                        scores_on_screen.append( [shuttlecock.score, row[0] + random.randint(0, int(row[2])), row[1] + random.randint(0, int(row[3])) , 0 , 255] )
    ...
    addDB(str(time_string), "挑戰模式", scoreBoard.score)
    pg.quit()
    # cap.release()
    return detection
