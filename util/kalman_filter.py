...
class Kalman_Filtering:
    ...
    def initialize(self):
        ...
        kalman.processNoiseCov = np.eye(n_states, dtype = np.float32)*0.003
    ...
    def predict(self,pg_points,ty):
        if ty == 'pg':
            points=[]
            points.append([pg_points[0], pg_points[1]])
            points.append([pg_points[0]+pg_points[2], pg_points[1]+pg_points[3]])
            points = np.array(points)
        else:
            points = np.array(pg_points)
        pred = []
